...
description: "Loads relevant context from the Hermes Memory Wiki at session start. Uses dialectic summaries from multi-pass LLM reasoning to inject rich, relevant context. Runs silently — no output unless context is injected."
---
...
- "resume", "cv", "cover letter", "document", "pdf" → Documents & Writing
...
- "agent", "multi-agent" → AI Agents

**Only inject if there's a clear match.** When in doubt, don't inject.

## Context injection

For each matching project, the wiki index contains `dialectic_context` — a rich summary generated by multi-pass LLM reasoning that captures:
- What was accomplished
- Key decisions and preferences
- Where things left off
- Important technical details

When injecting, prefer the `dialectic_context` over the basic `summary`. Format:

```
📚 **Previous Work: {Project Name}**
{dialectic_context, trimmed to ~3-4 sentences}
Last worked: {date}.
```

Keep total injection under ~150 words. Include at most 2-3 project summaries.
